...
from car_detection.custom_layers import yolov4_neck, yolov4_head, nms
from car_detection.utils import load_weights, get_detection_data, draw_bbox, voc_ap, draw_plot_func, read_txt_to_list
from car_detection.config import yolo_config
from car_detection.loss import yolo_loss
...
class Yolov4(object):
    ...
    def predict_only_cars(self, input_im):
        raw_img, untouched_img = input_im, input_im
        imgs = None
        if type(input_im) == str:
            untouched_img = cv2.imread(input_im)
            raw_img = untouched_img[:, :, ::-1]
            img = self.preprocess_img(raw_img)
            imgs = np.expand_dims(img, axis=0)
        else:
            imgs = np.expand_dims(self.preprocess_img(input_im), axis=0)

        pred_output = self.inference_model.predict(imgs)
        detections = get_detection_data(img=raw_img,
                                        model_outputs=pred_output,
                                        class_names=self.class_names)
        only_cars = detections.loc[detections['class_name'] == 'car']
        images_buffer = []
        for row in only_cars.index:
            x1, y1, x2, y2 = only_cars['x1'][row], only_cars['y1'][row], only_cars['x2'][row], only_cars['y2'][row]
            tmp = untouched_img[y1:y2, x1:x2, :]
            tmp = np.dstack((np.reshape(tmp[:, :, 2], (tmp.shape[0], tmp.shape[1], 1)),
                             np.reshape(tmp[:, :, 1], (tmp.shape[0], tmp.shape[1], 1)),
                             np.reshape(tmp[:, :, 0], (tmp.shape[0], tmp.shape[1], 1))))
            images_buffer.append(tmp)
